...
(function(global, factory) {
  ...
  ***************************************************************************** */
  ...
  function setTransform(elem, _a, _options) {
    var x = _a.x; var y = _a.y; var scale = _a.scale; var isSVG = _a.isSVG;
  ...
  function Panzoom(elem, options) {
    ...
    function move(event) {
      ...
      if (!isPanning) {
        ...
        var panDiff = Math.hypot(current.clientX - startClientX, current.clientY - startClientY);
        if (Math.abs(scaleDiff) > options.minDistance || panDiff > options.minDistance) {
          trigger('panzoomstart', {origX: origX, origY: origY, startScale: startScale}, options);
          isPanning = true;
        }
      }
      if (isPanning) {
        if (pointers.length > 1) {
          var toScale = constrainScale((scaleDiff * options.step) / 80 + startScale).scale;
