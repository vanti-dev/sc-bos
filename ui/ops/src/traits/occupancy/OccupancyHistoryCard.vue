...
<template>
  <v-card class="history-card">
    <v-list-subheader class="text-title-caps-large text-neutral-lighten-3">Occupancy History</v-list-subheader>
...
<script>
import {occupancyRecordToObject, useListOccupancyHistory} from '@/traits/occupancy/occupancy.js';
...
export default {
  ...
  methods: {
    async downloadHistory(n) {
      ...
      while (true) {
        const response = await useListOccupancyHistory(baseRequest);
