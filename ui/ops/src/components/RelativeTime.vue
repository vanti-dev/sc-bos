...
const absoluteFormatter = new Intl.DateTimeFormat(undefined, {hour: '2-digit', minute: '2-digit', second: '2-digit'});

const timeObj = computed(() => {
  if (typeof props.time === 'number') return new Date(props.time);
  if (typeof props.time === 'string') return new Date(Date.parse(props.time));
  return props.time;
});
