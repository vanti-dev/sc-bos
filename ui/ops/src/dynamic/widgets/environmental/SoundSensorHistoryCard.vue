...
import SoundSensorHistoryChart from '@/dynamic/widgets/environmental/SoundSensorHistoryChart.vue';
...
const onDownloadClick = async () => {
  ...
  await triggerDownload(
      props.title?.toLowerCase()?.replace(' ', '-') ?? 'sound-level',
      conditions,
      {startTime: startDate.value, endTime: endDate.value},
      {
        includeColsList: [
          ...
          {name: 'sound.pressurelevel', title: 'Sound Pressure Level (dB)'},
          {name: 'sound.frequency', title: 'Frequency (Hz)'},
          {name: 'sound.decibela', title: 'A-weighted Decibels (dBA)'},
          {name: 'sound.decibelc', title: 'C-weighted Decibels (dBC)'},
        ]
      }
  )
}
