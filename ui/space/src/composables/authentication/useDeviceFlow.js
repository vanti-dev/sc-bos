...
export default function useDeviceFlow(config) {
   ...
   *
   * @param {number} [leeway] If the token expires within this many seconds, refresh it.
   * @return {Promise<AuthenticationDetails>}
   */
  const refreshToken = async (leeway = 5) => {
    ...
    if (!isAccessTokenExpired(oldTokenResponse, leeway)) return tokenResponseToAuthDetails(oldTokenResponse);
  ...
  // Handle auto refreshing the token, handles the case where no API requests (and hence no refreshToken calls)
  // are happening for a while.
  const autoRefreshHandle = ref(0);
  const autoRefreshAt = computed(() => {
    if (!tokenResponse.value) return null;
    return new Date(tokenResponse.value.timestamp + tokenResponse.value.refresh_expires_in * 1000 - 5 * SECOND);
  });
  watch(autoRefreshAt, () => {
    clearTimeout(autoRefreshHandle.value);
    const refreshAt = autoRefreshAt.value;
    if (!refreshAt) return;
    const refreshIn = refreshAt.getTime() - Date.now();
    autoRefreshHandle.value = setTimeout(() => {
      refreshToken()
          .catch(e => console.warn('auto refresh token error', e));
    }, refreshIn);
  });
