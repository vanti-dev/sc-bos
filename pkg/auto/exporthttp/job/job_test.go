...
func Test_shouldExecuteImmediately(t *testing.T) {
	var tests = []struct {
	...
	}{
		{
			...
			now:      makeTime("08:59").Add(24 * time.Hour),
		...
		},
		{
			...
			schedule: jsontypes.MustParseSchedule("0 9 * * *"),
			now:      makeTime("08:00").Add(24 * time.Hour),
			previous: makeTime("09:00"),
			expected: false,
		},
		{
		...
		},
		{
			...
			schedule: jsontypes.MustParseSchedule("0 9 * * *"),
			now:      makeTime("08:59").Add(48 * time.Hour),
			previous: makeTime("09:00"),
			expected: true,
		},
		{
			...
			schedule: jsontypes.MustParseSchedule("0 9 * * *"),
			now:      makeTime("09:01").Add(48 * time.Hour),
			previous: makeTime("09:00"),
			expected: true,
		},
		{
			...
			schedule: jsontypes.MustParseSchedule("0 9 * * *"),
			now:      makeTime("09:00").Add(12 * time.Hour),
			previous: makeTime("09:00"),
			expected: false,
		},
		{
			...
			schedule: jsontypes.MustParseSchedule("0 9 * * *"),
			now:      makeTime("09:00").Add(13 * time.Hour),
			previous: makeTime("09:00"),
			expected: false,
		},
		{
