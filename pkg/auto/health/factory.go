// Package health provides an automation that watches device traits for values that exceed normal bounds and updates health checks based on the results.
package health
...
import (
	...
	"github.com/vanti-dev/sc-bos/internal/protobuf/protopath2"
	"github.com/vanti-dev/sc-bos/pkg/auto"
	"github.com/vanti-dev/sc-bos/pkg/auto/health/config"
	"github.com/vanti-dev/sc-bos/pkg/auto/health/internal/anytrait"
	...
	go func() {
		...
		_ = task.Run(ctx, func(ctx context.Context) (task.Next, error) {
			stream, err := devicesApi.PullDevices(ctx, &gen.PullDevicesRequest{
				...
				Query:    &gen.Device_Query{Conditions: cfg.Devices},
			})
			if err != nil {
			...
			for {
				...
				for _, change := range res.GetChanges() {
					...
					case ov == nil && nv != nil:
						...
						stop, err := a.newCheck(ctx, nv, cfg.Check, cfg.Source)
						if err != nil {
...
func (a *impl) newCheck(ctx context.Context, device *gen.Device, checkCfg *gen.HealthCheck, source config.Source) (func(), error) {
	...
	if err != nil {
		return nil, fmt.Errorf("%w: %s", err, source.Trait)
	}
	...
	rpath, err := protopath2.ParsePath(r.Message(), string(source.Value))
	if err != nil {
	...
	fetcher := resourceFetcher(a.Node.ClientConn(), anytrait.ReadRequest{
		Name:     device.Name,
		ReadMask: source.Value.ToFieldMask(),
	}, r)
	g.Go(func() error {
		defer close(changes)
	...
	g.Go(func() error {
		for change := range changes {
			...
			currentVal := values.Index(-1).Value
			healthVal, err := healthValueFromReflectValue(currentVal)
			if err != nil {
				logger.Debug("health value conversion failed", zap.Any("currentVal", currentVal), zap.Error(err))
...
func healthValueFromReflectValue(v protoreflect.Value) (*gen.HealthCheck_Value, error) {
	switch goValue := v.Interface().(type) {
	case nil:
		return nil, nil
	case bool:
