...
func CheckRangeState(r *gen.HealthCheck_ValueRange, v *gen.HealthCheck_Value, current gen.HealthCheck_Check_State) gen.HealthCheck_Check_State {
	switch current {
	case gen.HealthCheck_Check_LOW:
		if low := r.GetLow(); low != nil && less(v, AddValues(low, r.GetDeadband())) {
			return gen.HealthCheck_Check_LOW
		}
		if high := r.GetHigh(); high != nil && less(high, v) {
			return gen.HealthCheck_Check_HIGH
		}
		return gen.HealthCheck_Check_NORMAL
	case gen.HealthCheck_Check_HIGH:
...
func (c *BoundsCheck) UpdateBounds(_ context.Context, b *gen.HealthCheck_Check) error {
	...
	c.write(func(dst *gen.HealthCheck) {
		c.checker = checker
		dst.Check.Bounds = b.Bounds
		dst.Check.DisplayUnit = b.DisplayUnit
		c.writeValue(dst, dst.GetCheck().GetCurrentValue()) // recheck the known value against the new bounds
	})
	return nil
}
...
func (c *BoundsCheck) writeValue(dst *gen.HealthCheck, v *gen.HealthCheck_Value) {
	...
	newState := c.checker.valueToState(v, check)
	check.State = newState
	check.CurrentValue = v
...
func (c *BoundsCheck) prepareChecker(b *gen.HealthCheck_Check) (boundsChecker, error) {
	switch v := b.GetBounds().(type) {
	case nil:
		return nil, nil // no bounds, no value checking
	case *gen.HealthCheck_Check_NormalValue:
		return newNormalValueCheck(b.CurrentValue, v.NormalValue)
	case *gen.HealthCheck_Check_NormalRange:
		return newNormalRangeCheck(b.CurrentValue, v.NormalRange)
	case *gen.HealthCheck_Check_NormalValues:
		return newNormalValuesCheck(b.CurrentValue, v.NormalValues.GetValues())
	case *gen.HealthCheck_Check_AbnormalValues:
		return newAbnormalValuesCheck(b.CurrentValue, v.AbnormalValues.GetValues())
	}
	return nil, fmt.Errorf("unsupported bounds type %T", b.GetBounds())
}

// boundsChecker compares a value against a normal state, returning the appropriate check state.
type boundsChecker interface {
	valueToState(v *gen.HealthCheck_Value, current *gen.HealthCheck_Check) gen.HealthCheck_Check_State
}

// normalValueCheck checks if a value matches a specific normal value.
type normalValueCheck struct {
...
func newNormalValueCheck(v, nv *gen.HealthCheck_Value) (*normalValueCheck, error) {
	...
	return &normalValueCheck{value: nv}, nil
}

func (c *normalValueCheck) valueToState(v *gen.HealthCheck_Value, _ *gen.HealthCheck_Check) gen.HealthCheck_Check_State {
	if proto.Equal(v, c.value) {
		return gen.HealthCheck_Check_NORMAL
	}
	return gen.HealthCheck_Check_ABNORMAL
}
...
func (c *normalRangeCheck) valueToState(v *gen.HealthCheck_Value, current *gen.HealthCheck_Check) gen.HealthCheck_Check_State {
	return CheckRangeState(c.bounds, v, current.GetState())
}

// normalValuesCheck checks if a value matches any of a set of normal values.
type normalValuesCheck struct {
	values []*gen.HealthCheck_Value
}

func newNormalValuesCheck(v *gen.HealthCheck_Value, vs []*gen.HealthCheck_Value) (*normalValuesCheck, error) {
	if err := validateValuesCheck(v, vs); err != nil {
		return nil, fmt.Errorf("normal %w", err)
	}
	return &normalValuesCheck{values: vs}, nil
}

func (c *normalValuesCheck) valueToState(v *gen.HealthCheck_Value, _ *gen.HealthCheck_Check) gen.HealthCheck_Check_State {
	return valuesToState(v, c.values, gen.HealthCheck_Check_NORMAL, gen.HealthCheck_Check_ABNORMAL)
}

// abnormalValuesCheck checks if a value matches any of a set of abnormal values.
type abnormalValuesCheck struct {
	values []*gen.HealthCheck_Value
}

func newAbnormalValuesCheck(v *gen.HealthCheck_Value, vs []*gen.HealthCheck_Value) (*abnormalValuesCheck, error) {
	if err := validateValuesCheck(v, vs); err != nil {
		return nil, fmt.Errorf("abnormal %w", err)
	}
	return &abnormalValuesCheck{values: vs}, nil
}

func (c *abnormalValuesCheck) valueToState(v *gen.HealthCheck_Value, _ *gen.HealthCheck_Check) gen.HealthCheck_Check_State {
	return valuesToState(v, c.values, gen.HealthCheck_Check_ABNORMAL, gen.HealthCheck_Check_NORMAL)
}
...
func valuesToState(v *gen.HealthCheck_Value, vs []*gen.HealthCheck_Value, eq, neq gen.HealthCheck_Check_State) gen.HealthCheck_Check_State {
	for _, nv := range vs {
		if proto.Equal(v, nv) {
			return eq
		}
