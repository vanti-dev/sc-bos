...
func (c *ANPRController) poll(ctx context.Context) error {
	...
	for {
		...
		res, err := c.client.ListVisitorAppointments(&api.ListVisitorAppointmentsRequest{
	...
	for _, cam := range c.conf.ANPRCameras {
		...
		for {
			resp, err := c.client.ListANPREvents(&api.ANPREventsRequest{
...
func (c *ANPRController) CreateAccessGrant(ctx context.Context, request *gen.CreateAccessGrantRequest) (*gen.AccessGrant, error) {
	...
	resp, err := c.client.CreateVisitorAppointment(req)
	...
	if manuallyApprove {
		...
		resp, err := c.client.ManuallyApproveVisitor(&api.VisitorApprovalRequest{
			OperateType:     0,
	...
	return &gen.AccessGrant{
		ReadOnly:  manuallyApprove, // warning: only if auto approval is enabled, we are permitted to update the appointment
...
func (c *ANPRController) UpdateAccessGrant(ctx context.Context, request *gen.UpdateAccessGrantRequest) (*gen.AccessGrant, error) {
	...
	resp, err := c.client.UpdateVisitorAppointment(req)
...
func (c *ANPRController) DeleteAccessGrant(ctx context.Context, request *gen.DeleteAccessGrantRequest) (*gen.DeleteAccessGrantResponse, error) {
	c.logger.Debug("deleting access Grant", zap.String("source", request.GetSource()))

	if request.GetAccessGrantId() == "" {
		return nil, status.Error(codes.InvalidArgument, "access Grant ID is required")
	}

	_, err := c.client.DeleteVisitorAppointment(&api.DeleteVisitorAppointmentRequest{AppointRecordID: request.GetAccessGrantId()})
...
// It prefers appointments that are closer to the current time, and if two appointments have the same start time,
